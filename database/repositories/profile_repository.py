...
def find_profile_by_user_id(user_id: int) -> dict | None:
    rows = get_db().execute(
        "SELECT * FROM profiles WHERE user_id = %s", (user_id,), fetch=True
    )
...
def create_profile(user_id: int) -> int:
    return get_db().execute(
        "INSERT IGNORE INTO profiles (user_id) VALUES (%s)", (user_id,)
    )
...
def update_profile_fields(user_id: int, fields: dict):
    ...
    get_db().execute(
        f"UPDATE profiles SET {set_clause} WHERE user_id = %s", tuple(values)
    )


def update_profile_completion(user_id: int, completion: int):
    get_db().execute(
...
def get_profile_completion(user_id: int) -> int:
    rows = get_db().execute(
        "SELECT profile_completion FROM profiles WHERE user_id = %s",
        (user_id,), fetch=True,
    )
    if rows:
...
def update_user_preferences(user_id: int, fields: dict):
