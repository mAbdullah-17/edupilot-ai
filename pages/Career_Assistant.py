...
def _build_career_prompt(profile: dict | None) -> str:
    ...
    if profile:
        ...
        if profile.get("current_institution"):
            info.append(f"Institution: {profile['current_institution']}")
...
with nav_cols[0]:
    if st.button(
        "Resume Analyzer",
        key="career_to_resume", use_container_width=True,
    ):
        st.switch_page("pages/Resume_Analyzer.py")
with nav_cols[1]:
    if st.button(
        "Interview Coach",
        key="career_to_interview", use_container_width=True,
    ):
...
with gen_col2:
    if st.button(
        "Regenerate",
        key="career_regenerate", use_container_width=True,
    ):
...
if existing and not st.session_state.get("_career_force_new"):
    ...
    for rec in existing:
        with st.container():
            ...
            if rec.get("skill_gaps"):
                st.markdown(f"**Skill Gaps:** {rec['skill_gaps']}")
            if rec.get("roadmap"):
                st.markdown(f"**Roadmap:** {rec['roadmap']}")
else:
    ...
    if "_career_generated" not in st.session_state:
        ...
        with st.spinner("Analysing your profile for career matches..."):
            try:
                result = ai.generate_structured_response(
                    prompt, feature="career"
                )
                # Parse the result
                recs = []
                ...
                for rec in recs:
                    if isinstance(rec, dict):
                        repo.save_career_recommendation(
                            user_id,
                            rec.get("career_title", "Career Option"),
                            rec.get("match_score"),
                            rec.get("explanation", ""),
                            rec.get("skill_gaps", ""),
                            rec.get("roadmap", ""),
...
if profile:
    ...
    info_items = []
    if profile.get("education_level"):
        info_items.append(f"Education: {profile['education_level']}")
    if profile.get("current_field"):
        info_items.append(f"Field: {profile['current_field']}")
    if profile.get("current_institution"):
        info_items.append(f"Institution: {profile['current_institution']}")
    if profile.get("country"):
        info_items.append(f"Country: {profile['country']}")
    if info_items:
    ...
    st.caption("Profile not found. Complete your profile setup for better results.")
